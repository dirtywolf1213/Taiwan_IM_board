...
def build_112():
    ...
    block(m, 156, 160, '皮膚')
    over = {
        73: '心臟血管',       # bicuspid aortic valve
        74: '胸腔',          # 咳嗽
        75: '消化',          # Barrett's esophagus
        76: '腎臟',          # 健檢蛋白尿/血尿
        77: '感染',          # 退伍軍人症
        78: '新陳代謝與內分泌',  # 尿崩症 DI
        79: '風濕免疫及過敏',    # 僵直性脊椎炎
        80: '血液腫瘤',       # cryoprecipitate
        145: '神經', 146: '神經', 147: '神經', 148: '神經', 149: '神經',
        150: '精神', 151: '精神', 152: '精神', 153: '精神', 154: '精神', 155: '精神',
    }
    m.update(over)
    return m
...
def build_111():
    m = {}
    block(m, 1, 18, '心臟血管')      # Q18 高血鉀 ECG 變化
    block(m, 19, 36, '胸腔')
    block(m, 37, 54, '消化')
    block(m, 55, 72, '腎臟')
    block(m, 81, 96, '感染')
...
def build_110():
    m = {}
    block(m, 1, 18, '心臟血管')      # Q18 Marfan syndrome
    block(m, 19, 36, '胸腔')
    block(m, 37, 54, '消化')
    block(m, 55, 72, '腎臟')
    block(m, 81, 96, '感染')
...
MAPS = {
    114: build_114(), 113: build_113(), 112: build_112(),
    111: build_111(), 110: build_110(),
}
