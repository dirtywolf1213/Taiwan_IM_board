...
- `topic`(選填)= 考點次分類(單一主考點,英文/縮寫,如 `ACS`/`HF`)。已分類科:心臟血管、感染、風濕免疫及過敏(其餘陸續補)。由 `tools/classify_topic.py` 依「題幹+選項+詳解」關鍵字自動分類(各科考點清單見該檔 `TAXO`);改動後重建 `index.json`(已含 `topic`,供 chips/搜尋免載整年)。UI:依科目練習的考點 chips、每題 badge、全題庫搜尋。
